/**
 * CompanyWSHandler.java
 * com.visa.companyzlr.websocket
 ...
 * @Date	 2017年9月29日 	 
 */
public class CompanyWSHandler implements WebSocketHandler {

	public static final Map<String, List<WebSocketSession>> companySocketSessionMap = Maps.newConcurrentMap();
	...
	public void afterConnectionEstablished(WebSocketSession wss) throws Exception {
		...
		List<WebSocketSession> wssLst = companySocketSessionMap.get(uid);
		if (Util.isEmpty(wssLst)) {
			wssLst = Lists.newArrayList();
			wssLst.add(wss);
			companySocketSessionMap.put(uid, wssLst);
		} else {
			wssLst.add(wss);
		}
	...
	*/
	public void broadcast(final TextMessage message) throws IOException {
		Set<String> keySet = companySocketSessionMap.keySet();
		if (!Util.isEmpty(keySet)) {
			Iterator<String> iter = keySet.iterator();
			while (iter.hasNext()) {
				String nextJ = iter.next();
				List<WebSocketSession> lst = companySocketSessionMap.get(nextJ);
				if (!Util.isEmpty(lst)) {
					//多线程群发  TODO
	...
	public static Map<String, List<WebSocketSession>> getUsersocketsessionmap() {
		return companySocketSessionMap;
	}
